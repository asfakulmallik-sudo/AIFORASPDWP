...
> **Disclaimer on UI paths:** Intune admin center navigation and setting labels are updated frequently by Microsoft. Paths below reflect the most recent known layout at time of writing. Any path flagged with ⚠️ should be verified live in the tenant before deployment, as Microsoft has a history of renaming or relocating these nodes (e.g. moves between "Device compliance" and "Endpoint security", or Settings Catalog re-categorisation).
...
- **UI path:** Intune admin center > Devices > Compliance > Policies > \[Windows 10 and later policy] > System Security > Encryption of data storage on device ⚠️ *(compliance node is separate from the BitLocker encryption profile under Endpoint security > Disk encryption — verify both still exist under these names).*
...
- **UI path:** Intune admin center > Devices > Compliance > Policies > \[policy] > Device Health > Secure Boot must be enabled on the device ⚠️ *(sometimes listed under System Security instead of Device Health depending on catalog version — check both).*
...
- **UI path:** Intune admin center > Devices > Compliance > Policies > \[policy] > Device Properties > Minimum OS version ⚠️ *(build number format and whether it accepts full 4-part version strings has changed before — confirm current accepted format live).*
...
- **UI path:** Intune admin center > Devices > Compliance > Policies > \[policy] > System Security > Microsoft Defender Antivirus > Require Real-time protection ⚠️ *(this setting has moved between "Defender" and "System Security" groupings across Intune UI revisions).*
...
- **UI path:** Devices > Compliance > Policies > \[policy] > System Security > Password Required / Minimum password length. Hello for Business PIN policy: Devices > Endpoint security > Account protection ⚠️ *(Account protection blade is a relatively newer addition — confirm it still exists under this name).*
...
- **UI path:** Devices > Compliance > Policies > \[policy] > Device Health > Require Code integrity (Windows substitute check) ⚠️ *(confirm exact wording — "Code integrity" enforcement setting naming has varied across catalog versions).*
...
- **UI path:** Devices > Compliance > Policies > \[policy] > Actions for noncompliance > Schedule (days after noncompliance) ⚠️ *(this tab has been relabelled "Actions for noncompliance" vs "Noncompliance actions" in different releases — confirm current name).*
...
**Recommendation before go-live:** Log into the live Intune admin center and screenshot/confirm each ⚠️ path, since this baseline was translated from training knowledge that may lag current UI releases.
